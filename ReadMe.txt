acTelerikStylesAssembly (22.05.2015), v1.9
Copyright (c) 2012-2015 acDevSoftware

Create a Telerik Skins assembly from various CSS Telerik zipped files created by hands, by
acTelerikStylesRenaming or by Telerik StyleBuilder web application (http://stylebuilder.telerik.com)

Any questions about the acTelerikStylesAssembly:
supports@acdevsoftware.ch?subject=[acTelerikStylesAssembly]
Or:  
http://www.acdevsoftware.ch/Supports/CustomersService/Default.aspx?cat=TOOLS&sct=GSUP

LinkedIn:
Follow the product news on LinkedIn:
http://www.linkedin.com/company/acdevsoftware

Facebook:
https://www.facebook.com/pages/Acdevsoftware/1375578242656685

Twitter:
https://twitter.com/acdevsoftware


Indexes:
A. Dependencies.
B. Installation.
C. Usage with GUI version of acTelerikStylesAssembly.
D. Usage with console version of acTelerikStylesAssembly
E. How to add the created Telerik Skins assembly in your ASP.NET web application/web site?
F. Example how to create a Telerik Skins assembly named Test.dll containing 3 different skins,
   with a default one.
G. Create a zipped Telerik skin used by acTelerikStylesAssembly based on Telerik Black skin.
H. Important information about Telerik.Web.UI.dll.
I. Troubleshooting
J. History.
...
- The Telerik.Web.UI.dll assembly, installed with a Telerik product.
  Version taken in account for controls: 2014.3.1209
- Framework .NET 4.0
...
Before starting acTelerikStylesAssembly, it is necessary to indicate the full path of the
Telerik.Web.UI.dll assembly.

Note:
- GUI Version 1.4 and newer of acTelerikStylesAssembly lets you to enter directly the path
  of Telerik.Web.UI.dll assembly in the main window, field "Telerik Dll Path".
- Console version 1.9 and newer of acTelerikStylesAssembly lets you to enter the path
  of Telerik.Web.UI.dll assembly with argument /T.

This path can ben given in the acTelerikStylesAssembly.exe.config file, in the value of
the key "TelerikWebUIPath":
...
It can also be set in a acTelerikStylesAssembly.xml settings file, under the node
"TelerikWebUIPath":
<?xml version="1.0" encoding="utf-8"?>
<Settings>
...
Last possibility is to put the Telerik.Web.UI.dll file in the same folder that
acTelerikStylesAssembly executable file.


C. Usage with GUI version of acTelerikStylesAssembly

1. Create several zipped skin files by hands, by acTelerikStylesRenaming (see index G) or
   by StyleBuilder Telerik Application (http://stylebuilder.telerik.com). One of them must
   have the same name that the name of the Telerik Skins assembly to create (without .zip)
   and must contains all Telerik controls. 
2. After launching acTelerikStylesAssembly from a Windows Explorer, click on the button
   "Add Styles Files...", and add all zipped skin files created in point 1.
3. Set an output directory for the Telerik Skins assembly to create.
4. Set the path of the Telerik Dll assembly (Telerik.Web.UI.dll); optional, see point B.
5. Set a name without dll extension for the Telerik Skins assembly to create (name must be
   one of the zipped skin filename without .zip); Name example: MyBlack.
6. Set a version for the Telerik Skins assembly to create.
7. Click on the button "Generate Assembly" and if no error occurs, the Telerik Skins assembly
   is created in the Output Directory.


D. Usage with console version of acTelerikStylesAssembly (experimental)

1. Create several zipped skin files by hands, by acTelerikStylesRenaming (see index G) or
   by StyleBuilder Telerik Application (http://stylebuilder.telerik.com). One of them must
   have the same name that the name of the Telerik Skins assembly to create (without .zip)
   and must contains all Telerik controls.
2. Launching acTelerikStylesAssembly from a console by giving the necessary arguments in
   uppercase with values (/H for help):
      /X : The path name of a Xml settings file, override all other command line arguments
      /A : The name of the assembly to generate, without dll extension
      /R : The version of the assembly to generate [x.x.x.x]
      /S : The path name of a zipped Telerik skin file (repetitive argument)
      /O : The directory name where to save the generated assembly
      /T : The directory name where to find the Telerik.Web.UI.dll (optional)
      /V : Verbose log, no log output except error and help  (optional)
   By example 1:
      acTelerikStylesAssembly /AMyAssembly /R12.1.1.1 /S"C:\My SecondTest\Test.zip" /SC:\MyTest /O"C:\My Test" /V
   By Example 2:
      acTelerikStylesAssembly /X"C:\My SecondTest\acTelerikStylesAssembly.xml"


E. How to add the created Telerik Skins assembly in your ASP.NET web application/web site?

1. The version of Telerik.Web.UI.dll assembly used in your ASP.NET web application/web site
   must be the same that the one used for acTelerikStylesAssembly in index B.
2. In the web.config, under the node configuration/appSettings, add the following keys:
   ...
   - Telerik.Skin: Name of the created Telerik Skins assembly (see C.5 or D.2 /A), to be used as
     default skin for all Telerik controls.
...
3. To use other skins containing in your created Telerik Skins assembly, add in the main page
   or master page of your web application/web site the Telerik control <telerik:RadSkinManager>.
...
1. The "Test" folder contains 3 zipped skin files created by StyleBuilder Telerik Application
   (version 28.03.2013, Telerik.Web.UI.dll: 2013.1.411):
...
5. Fill the "Telerik Dll Path" field with the path to the "Telerik.Web.UI.dll".
...
G. Create a zipped Telerik skin file used by acTelerikStylesAssembly based on Telerik Black skin

By acTelerikStylesRenaming:
1. Follow the index C of the acTelerikStylesRenaming Readme.txt file.
...
4. Copy the content of the folder Common into your folder MySkin/Common.
...
8. Zip the content of the folder MySkin, and name it MySkin.zip.
   (the MySkin.zip tree must not content MySkin folder at the root, but directly
    files and folders of it)


H. Important information about Telerik.Web.UI.dll.

Until version 2014.3.1209 included (new versions not checked), some references to skin files are
hardcoded in Telerik.Web.UI.dll and don't depend of the appSetting "Telerik.Web.SkinsAssembly".
The result is when you use a custom Skin Assembly and a Telerik control uses these hardcoded
references on files, these files are not displayed because Telerik.Web.UI.dll is searching
a file for a control as "Telerik.Web.UI.Skins.<Skin Name>.<Control>.<Resource filename>" and not
as expected "<Skin Assembly Name>.<Skin Name>.<Control>.<Resource filename>. Hopefully,
there are only few hardcoded references:

RadGrid: Only the column GridEditCommandColumn ButtonType="ImageButton" gives a invalid
         references on files for Edit.gif, Update.gif, Cancel.gif.
RadDataPager: Images of SEO buttons, like PagingFirst.gif, etc.

As workarounds:
- Name your Skin Assembly "Telerik.Web.UI.Skins" and replace the Telerik one by yours in
  your project.
- For RadGrid, use the ImagesPath property to give a path for the image files like Edit.gif, etc.
  (workaround given by Samuel Chou).
...
Before v1.9, acTelerikStylesAssembly didn't know how to handle these comments and raised an error
when it processed the files containing them. So please remove these comments of the files in error
and acTelerikStylesAssembly v1.8 and earlier versions will handle them correctly.
...
Fix:
- Test/Test.zip, Test/Test_Ext1.zip and Test/Test_Ext2.zip have been generated with lasted
